...
function SmartImage({ src, alt = "" }: { src: string; alt?: string }) {
  return (
    <div style={{ width: "100%", height: "100%", position: "relative", overflow: "hidden", background: "#080812" }}>
      <img src={src} alt={alt} style={{ position: "absolute", width: "100%", height: "100%", objectFit: "contain", objectPosition: "center center", display: "block", transition: "opacity 0.3s" }} />
    </div>
  );
}
...
function PhotoGallery({ accent, photos, theme = "dark" }: { accent: string; photos: { url: string }[]; theme?: Theme }) {
  ...
  const headerBg = "linear-gradient(180deg,rgba(255,255,255,0.06) 0%,rgba(255,255,255,0.02) 100%)";
  const headerBorder = "rgba(255,255,255,0.07)";
  const navBtnBg = "rgba(8,8,18,0.68)";
  const navBtnBorder = "rgba(255,255,255,0.12)";
  const navBtnColor = "rgba(255,255,255,0.85)";
  ...
  return (
    <div style={{ width: "100%", height: "100%", position: "relative" }}>
      ...
      <div
        style={{ position: "relative", width: "100%", height: "100%", borderRadius: 22, overflow: "hidden", border: `1px solid ${frameBorder}`, boxShadow: ["0 32px 80px rgba(0,0,0,0.82)", "0 8px 24px rgba(0,0,0,0.55)", `inset 0 1px 0 ${frameInset}`].join(","), background: frameBg, zIndex: 1, display: "flex", flexDirection: "column" }}
        ...
        <div style={{ flex: 1, position: "relative", overflow: "hidden", minHeight: 0 }}>
          ...
          {photos.length > 1 && (
            <div style={{ position: "absolute", bottom: 12, left: "50%", transform: "translateX(-50%)", display: "flex", gap: 5, zIndex: 10 }}>
              {photos.map((_, i) => (
                <button key={i} onClick={() => setCurrent(i)} style={{ width: i === current ? 18 : 5, height: 5, borderRadius: 99, background: i === current ? accent : dotInactive, border: "none", cursor: "pointer", padding: 0, transition: "all 0.30s cubic-bezier(0.16,1,0.3,1)", boxShadow: i === current ? `0 0 8px ${accent}` : "none" }} />
              ))}
            </div>
          )}
        </div>
...
export const SERVICE_DATA = [
  {
    ...
    features: [
      { icon: "⚡", label: "Sub-second load times" }, { icon: "◈", label: "Corporate websites" },
      { icon: "🛒", label: "E-Commerce ready" }, { icon: "🚀", label: "CMS solutions" },
      { icon: "◎", label: "Landing pages" }, { icon: "✓", label: "SEO optimized" },
    ],
  ...
  },
  {
    ...
    features: [
      { icon: "✦", label: "Figma design" }, { icon: "◎", label: "Wireframing" },
      { icon: "▣", label: "Prototyping" }, { icon: "◈", label: "Design systems" },
      { icon: "◑", label: "Dark / light mode" }, { icon: "✓", label: "WCAG AA" },
    ],
  ...
  },
  {
    ...
    features: [
      { icon: "⚡", label: "Next.js + TypeScript" }, { icon: "◈", label: "REST & GraphQL APIs" },
      { icon: "💳", label: "Payments & billing" }, { icon: "🔐", label: "Auth & RBAC" },
      { icon: "◉", label: "Real-time features" }, { icon: "🚀", label: "Edge deployment" },
    ],
  ...
  },
  {
    ...
    features: [
      { icon: "◆", label: "AI chatbots" }, { icon: "🔍", label: "RAG pipelines" },
      { icon: "🤖", label: "Automation flows" }, { icon: "⚡", label: "OpenAI / Claude" },
      { icon: "⟳", label: "Custom AI tools" }, { icon: "◉", label: "n8n workflows" },
    ],
...
function AnimatedBackground({ accent, rgb, theme = "dark" }: { accent: string; rgb: string; theme?: Theme }) {
  ...
  return (
    <div style={{ position: "absolute", inset: 0, overflow: "hidden", pointerEvents: "none" }}>
      <div style={{ position: "absolute", inset: 0, background: isDark ? `radial-gradient(ellipse 80% 70% at 78% 38%, rgba(${rgb},0.14) 0%, transparent 60%), radial-gradient(ellipse 55% 45% at 15% 72%, rgba(${rgb},0.08) 0%, transparent 55%), radial-gradient(ellipse 40% 35% at 50% 8%, rgba(${rgb},0.06) 0%, transparent 50%)` : `radial-gradient(ellipse 80% 70% at 78% 38%, rgba(${rgb},0.10) 0%, transparent 60%), radial-gradient(ellipse 55% 45% at 15% 72%, rgba(${rgb},0.06) 0%, transparent 55%), radial-gradient(ellipse 40% 35% at 50% 8%, rgba(${rgb},0.04) 0%, transparent 50%)`, transition: "background 0.6s ease" }} />
      {orbs.map((orb, i) => (
      ...
      <div style={{ position: "absolute", inset: 0, backgroundImage: `linear-gradient(rgba(${rgb},${isDark ? "0.04" : "0.03"}) 1px, transparent 1px), linear-gradient(90deg, rgba(${rgb},${isDark ? "0.04" : "0.03"}) 1px, transparent 1px)`, backgroundSize: "80px 80px", maskImage: "radial-gradient(ellipse 85% 85% at 50% 50%, black 30%, transparent 100%)", WebkitMaskImage: "radial-gradient(ellipse 85% 85% at 50% 50%, black 30%, transparent 100%)" }} />
      <div style={{ position: "absolute", top: "-30%", right: "26%", width: 1.5, height: "160%", background: `linear-gradient(180deg, transparent, rgba(${rgb},${isDark ? "0.20" : "0.12"}) 35%, rgba(${rgb},${isDark ? "0.12" : "0.07"}) 65%, transparent)`, transform: "rotate(16deg)" }} />
      {isDark && <div style={{ position: "absolute", inset: 0, backgroundImage: "repeating-linear-gradient(0deg, transparent, transparent 3px, rgba(255,255,255,0.007) 3px, rgba(255,255,255,0.007) 4px)" }} />}
      <div style={{ position: "absolute", inset: 0, background: isDark ? "radial-gradient(ellipse 100% 100% at 50% 50%, transparent 35%, rgba(0,0,0,0.65) 100%)" : "radial-gradient(ellipse 100% 100% at 50% 50%, transparent 35%, rgba(255,255,255,0.50) 100%)" }} />
...
export function ServiceSlide({ svcIndex, theme = "dark" }: { svcIndex: number; theme?: Theme }) {
  ...
  const ruleGradient = isDark ? `linear-gradient(90deg,${accentColor},${accentColor}30)` : `linear-gradient(90deg,${accentColor},${accentColor}50)`;
    ...
    <div style={{ position: "relative", width: "100%", minHeight: "100vh", display: "flex", alignItems: isMobile ? "flex-start" : "center", background: sectionBg, overflow: "hidden", transition: "background 0.4s ease" }}>
      ...
      <div style={{ position: "relative", zIndex: 10, maxWidth: 1400, margin: "0 auto", width: "100%", padding: isMobile ? "5rem 1.25rem 4rem" : "0 3.5rem", display: isMobile ? "flex" : "grid", flexDirection: isMobile ? "column" : undefined, gridTemplateColumns: isMobile ? undefined : "0.85fr 1.15fr", gap: isMobile ? "2rem" : "5rem", alignItems: "center" }}>
        ...
        <motion.div key={`l-${svcIndex}-${theme}`} initial={{ opacity: 0, x: isMobile ? 0 : -28, y: isMobile ? 20 : 0 }} animate={{ opacity: 1, x: 0, y: 0 }} transition={{ duration: 0.65, ease: [0.22, 1, 0.36, 1] }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: isMobile ? 6 : 7, marginBottom: isMobile ? 24 : 36, maxWidth: isMobile ? "100%" : 400 }}>
            {svc.features.map((f, idx) => (
              <motion.div key={f.label} initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4, delay: 0.2 + idx * 0.055 }} style={{ display: "flex", alignItems: "center", gap: 9, padding: isMobile ? "8px 10px" : "9px 12px", borderRadius: 10, border: `1px solid ${featureBorder}`, background: featureBg, backdropFilter: "blur(16px)", WebkitBackdropFilter: "blur(16px)" }}>
