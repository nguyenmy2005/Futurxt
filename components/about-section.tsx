...
const CD = {
  keyword: "rgba(255,255,255,0.98)", string: "#7DD3C8", number: "#FFD580",
  comment: "rgba(255,255,255,0.35)", function: "#A8D4FF", type: "#D4AAFF",
  variable: "rgba(255,255,255,0.88)", punctuation: "rgba(255,255,255,0.50)",
  operator: "rgba(255,255,255,0.60)", plain: "rgba(255,255,255,0.82)",
};
const CL = {
  keyword: "#1a1a2e", string: "#0d7377", number: "#b45309",
  comment: "rgba(0,0,0,0.35)", function: "#1d4ed8", type: "#7c3aed",
  variable: "rgba(0,0,0,0.80)", punctuation: "rgba(0,0,0,0.45)",
  operator: "rgba(0,0,0,0.55)", plain: "rgba(0,0,0,0.75)",
};

function syntaxColor(line: string, lang: string, isDark: boolean): React.ReactNode {
  ...
  if (rest.startsWith("//") || rest.startsWith("#") || rest.startsWith("/*")) { push(rest, C.comment, true); return <>{parts}</>; }
  if (rest.startsWith("#[") || rest.startsWith("@")) { push(rest, C.type); return <>{parts}</>; }
...
const EXTS: Record<string, string> = { TypeScript: ".ts", Python: ".py", Go: ".go", Rust: ".rs", Kotlin: ".kt", Swift: ".swift" };
...
function CodeEditor() {
  ...
  useEffect(() => {
    if (done) return;
    if (visibleLines >= lines.length) { setDone(true); setTimeout(() => setLangIdx(p => (p + 1) % languages.length), 3200); return; }
    const currentLine = lines[visibleLines];
    if (charIdx < currentLine.length) { const t = setTimeout(() => setChar(c => c + 1), 18); return () => clearTimeout(t); }
    const t = setTimeout(() => { setVisible(v => v + 1); setChar(0); }, 25);
    return () => clearTimeout(t);
  }, [visibleLines, charIdx, done, lines, languages.length]);
  useEffect(() => { if (scrollRef.current) scrollRef.current.scrollTop = scrollRef.current.scrollHeight; }, [visibleLines, charIdx]);
  ...
  return (
    <div style={{ width: "100%", borderRadius: 20, border: `1px solid ${editorBorder}`, background: editorBg, backdropFilter: "blur(60px) saturate(200%) brightness(1.1)", WebkitBackdropFilter: "blur(60px) saturate(200%) brightness(1.1)", overflow: "hidden", boxShadow, transition: "background 0.5s ease, border 0.5s ease", height: "clamp(380px, 50vw, 420px)", display: "flex", flexDirection: "column" }}>
      <div style={{ display: "flex", alignItems: "center", padding: "12px 16px", background: titleBarBg, borderBottom: `1px solid ${titleBarBorder}`, gap: 10, transition: "background 0.5s ease", flexShrink: 0 }}>
        ...
        <div style={{ flex: 1, display: "flex", justifyContent: "center" }}>
          <span style={{ fontSize: 12, fontFamily: "monospace", color: fileNameColor, background: fileNameBg, padding: "3px 14px", borderRadius: 99, border: `1px solid ${fileNameBorder}`, letterSpacing: "0.02em", transition: "color 0.5s ease" }}>
            futurxt{EXTS[lang]}
          </span>
        </div>
        <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
          <motion.div animate={{ opacity: done ? 1 : 0.7, backgroundColor: done ? "rgba(40,200,64,1)" : "rgba(255,189,46,1)" }} style={{ width: 7, height: 7, borderRadius: "50%" }} />
          <span style={{ fontSize: 11, fontFamily: "monospace", color: statusColor, transition: "color 0.5s ease" }}>{done ? "compiled" : "typing..."}</span>
        </div>
      </div>
      <div style={{ display: "flex", background: tabsBg, borderBottom: `1px solid ${tabsBorder}`, overflowX: "auto", scrollbarWidth: "none", transition: "background 0.5s ease", flexShrink: 0 }}>
        {languages.map((l, i) => (
          <button key={l} onClick={() => setLangIdx(i)} style={{ position: "relative", padding: "8px 14px", fontSize: 11, fontFamily: "monospace", color: i === langIdx ? tabActiveColor : tabInactiveColor, background: i === langIdx ? tabActiveBg : "transparent", border: "none", borderRight: `1px solid ${tabDivider}`, cursor: "pointer", whiteSpace: "nowrap", transition: "all 0.15s", fontWeight: i === langIdx ? 600 : 400, flexShrink: 0 }}>
            {l}
            {i === langIdx && <motion.div layoutId="tab-line" style={{ position: "absolute", bottom: 0, left: 0, right: 0, height: 2, background: tabLineColor, borderRadius: 2 }} />}
          </button>
        ))}
      </div>
      <div ref={scrollRef} style={{ padding: "16px 0", flex: 1, overflowY: "auto", overflowX: "auto", background: "transparent", scrollbarWidth: "thin", scrollbarColor: `${scrollbarColor} transparent` }}>
        {lines.map((line, li) => {
          if (li > visibleLines) return null;
          const isCurrent = li === visibleLines && !done;
          const displayLine = isCurrent ? line.slice(0, charIdx) : line;
          return (
            <div key={li} style={{ display: "flex", lineHeight: "24px", minHeight: 24, background: isCurrent ? currentLineBg : "transparent" }}>
              <span style={{ width: 40, paddingRight: 12, fontSize: 11, fontFamily: "monospace", color: isCurrent ? lineNumActive : lineNumInactive, userSelect: "none", flexShrink: 0, textAlign: "right", transition: "color 0.5s ease" }}>{li + 1}</span>
              <span style={{ fontSize: 12.5, fontFamily: "'JetBrains Mono','Fira Code','Cascadia Code',monospace", paddingRight: 16, whiteSpace: "pre", letterSpacing: "0.01em" }}>
                {syntaxColor(displayLine, lang, isDark)}
                {isCurrent && (
                  <motion.span animate={{ opacity: [1, 0, 1] }} transition={{ duration: 0.5, repeat: Infinity }} style={{ display: "inline-block", width: 2, height: 14, background: cursorColor, marginLeft: 1, verticalAlign: "middle", borderRadius: 1 }} />
                )}
              </span>
            </div>
          );
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "6px 16px", background: statusBarBg, borderTop: `1px solid ${statusBarBorder}`, transition: "background 0.5s ease", flexShrink: 0 }}>
        <span style={{ fontSize: 11, fontFamily: "monospace", color: statusColor }}>⎇ main</span>
        <span style={{ fontSize: 11, fontFamily: "monospace", color: statusColor, letterSpacing: "0.18em" }}>{lang.toUpperCase()}</span>
      </div>
    </div>
  );
}
...
export function AboutSection() {
  ...
  const topRule      = isDark ? "linear-gradient(90deg,transparent,rgba(255,255,255,0.10),transparent)" : "linear-gradient(90deg,transparent,rgba(0,0,0,0.10),transparent)";
  const glow         = isDark ? "radial-gradient(ellipse,rgba(255,255,255,0.04) 0%,transparent 70%)" : "radial-gradient(ellipse,rgba(0,0,0,0.03) 0%,transparent 70%)";
      ...
      <div className="relative z-10 w-full max-w-[1280px] mx-auto px-5 sm:px-8 lg:px-12 pt-20 sm:pt-24 pb-2">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-10 lg:gap-16 items-start mb-8 sm:mb-10">
          <motion.div initial={{ opacity: 0, y: 28 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.75, ease: [0.16, 1, 0.3, 1] }} className="flex flex-col">
            ...
            <h2 style={{ fontFamily: "'Georgia','Times New Roman',serif", fontSize: "clamp(2rem, 5vw, 3.8rem)", fontWeight: 900, lineHeight: 1.06, letterSpacing: "-0.03em", color: headingColor, margin: "0 0 28px", textShadow: headingShadow, transition: "color 0.5s ease, text-shadow 0.5s ease" }}>
              Built by engineers<br /><span>Designed for growth</span>
            </h2>
            <div style={{ display: "flex", flexDirection: "column", gap: 0, background: cardBg, border: `1px solid ${cardBorder}`, borderRadius: 20, backdropFilter: "blur(60px) saturate(200%) brightness(1.1)", WebkitBackdropFilter: "blur(60px) saturate(200%) brightness(1.1)", overflow: "hidden", boxShadow: cardShadow, transition: "background 0.5s ease, border 0.5s ease" }}>
              {paragraphs.map((p, i) => (
                <div key={i} style={{ padding: "20px 24px", borderBottom: i < 2 ? `1px solid ${rowBorder}` : "none", display: "flex", alignItems: "center", position: "relative" }}>
                  <div style={{ position: "absolute", left: 0, top: "20%", bottom: "20%", width: 2, borderRadius: 2, background: accentBars[i], transition: "background 0.5s ease" }} />
                  <p style={{ fontSize: "clamp(13px, 1.4vw, 15.5px)", lineHeight: 1.80, color: isDark ? `rgba(255,255,255,${p.opacity})` : `rgba(10,10,10,${p.opacity})`, margin: 0, fontWeight: p.weight, letterSpacing: "0.01em", transition: "color 0.5s ease" }}>{p.text}</p>
                </div>
              ))}
            </div>
          </motion.div>
